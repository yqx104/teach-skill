...
- `teach`：入口路由器与长期复习管理器。使用 `grilling` 逐题确认目标和模式，维护知识点掌握度，并按状态生成复习考试或复习课件。
...
- `LEARNER-PROFILE.md`：高层能力摘要、误解、环境和教学偏好。
- `MASTERY-LEDGER.md`：逐知识点的掌握等级、到期状态、最近证据和下次复习日期。
- `OUTLINE.md`：当前模式的课程目录。
- `RESOURCES.md`：课程依赖的高可信来源及覆盖范围。
- `lessons/*.html`：自包含课程。
- `quiz/*.md`：用户学完并明确表示准备好后生成的测验。
- `reviews/*.md` 和 `reviews/*.html`：按知识点状态生成的复习考试或个性化复习课件。
...
5. 创建掌握度账本，并在规划目录后补齐稳定的知识点 ID。
6. 路由到对应执行 skill 规划课程目录。
7. 后续一次只生成一节 lesson。
8. 用户学习并讨论后，明确说“可以考我了”才生成 quiz。

长期使用时，没有证据的知识点先记为 `U 待确认`；获得证据后采用五级状态：`L0 陌生`、`L1 已接触`、`L2 初步理解`、`L3 可独立运用`、`L4 熟练掌握`。复习状态另记为`未到期`、`到期`或`疑似遗忘`：时间到期只触发复习，不会自动降低掌握等级。

复习日期在每次调用 `teach` 或执行 skill 时刷新；这套 skill 本身不在后台发送提醒。

用户说“我想复习”时，`teach` 会优先选择疑似遗忘、到期核心前置和近期课程所需知识点，并让用户选择：

- **考试**：闭卷提取、应用和诊断，用表现证据更新等级与下次复习日期。
- **复习课件**：针对遗忘点生成自包含 HTML，帮助重新建立模型；阅读本身不算掌握证据。

课堂提问若与旧等级冲突，先记为“疑似遗忘”。只有一个短的定向检查确认后才降级，避免把正常追问或口误误判为遗忘。
